...
export class ReportControlElementEditor extends ScopedElementsMixin(
  ...
  @query('.save.optfields') optFieldsSave!: OscdTextButton;

  @queryAll('.content.trgops > oscd-scl-checkbox')
  trgOpsInputs!: OscdSclCheckbox[];

  @query('.save.trgops') trgOpsSave!: OscdTextButton;

  @queryAll('.report.attributes')
  reportControlInputs!: (OscdSclTextField | OscdSclSelect | OscdSclCheckbox)[];

  @query('.content.reportcontrol > .save') reportControlSave!: OscdTextButton;

  @query('.rptenabled.attributes') rptEnabledInput!: OscdSclTextField;
  ...
  private saveReportControlChanges(): void {
    ...
    if (!rptEnabledAction)
      {this.dispatchEvent(
        newEditEventV2(reportControlActions, {
          title: `Update ReportControl ${this.element}`,
        }),
      );}
    else
      {this.dispatchEvent(
        newEditEventV2([...reportControlActions, rptEnabledAction], {
          title: `Update ReportControl ${this.element}`,
        }),
      );}
          ...
          ([key, value]) =>
            html`<oscd-scl-checkbox
              ...
              @input=${async (evt: Event) => {
                await (evt.target as OscdSclCheckbox).updateComplete;
                this.onTrgOpsInputChange();
              }}
            ></oscd-scl-checkbox>`,
        )}
