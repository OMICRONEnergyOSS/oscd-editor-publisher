...
export class GseControlElementEditor extends ScopedElementsMixin(LitElement) {
  ...
  @queryAll('.content.gse > oscd-scl-text-field') gSEInputs!: OscdSclTextField[];

  @query('.content.gse > .save') gseSave!: OscdTextButton;
  ...
  @query('.content.gsecontrol > .save') gseControlSave!: OscdTextButton;

  @query('#instType') instType?: OscdCheckbox;
  ...
  private saveGSEChanges(): void {
    ...
    this.dispatchEvent(
      newEditEventV2(changeGSEContent(this.gSE, options), {
